	...
	for i := 0; i < count; i++ {
		testBilling := &dto.Billing{
			...
			BillingDetails: dto.BillingDetails{
				...
				TokenUsage:   map[string]int{"gpt-3.5-turbo": rand.Intn(1000000)},
				RequestCount: map[string]int{"gpt-3.5-turbo": rand.Intn(1000)},
			},
			QuotaDetails: dto.QuotaDetails{
				QuotaUsage:     map[string]float64{"default": float64(rand.Intn(100000)) / 100},
				QuotaRemaining: map[string]float64{"default": float64(rand.Intn(100000)) / 100},
			},
		}
