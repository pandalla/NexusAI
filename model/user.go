...
type User struct {
	...
	LastLoginTime *time.Time  `gorm:"column:last_login_time" json:"last_login_time"`                           // 最后一次成功登录的时间
	...
	CreatedAt time.Time      `gorm:"column:created_at;not null;default:CURRENT_TIMESTAMP(3)" json:"created_at"`                                // 用户账号的创建时间
	UpdatedAt time.Time      `gorm:"column:updated_at;not null;default:CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)" json:"updated_at"` // 用户信息的最后更新时间
